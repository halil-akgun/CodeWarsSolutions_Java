...
The first candidate got 2 votes. Even if all the remaining 3 candidates vote for him, he will still have only 5 votes,
 ...
 */
public class CW23_ElectionsWinners {
    public static void main(String[] args) {
        int[] votes = {2, 3, 5, 2};
        int remainder = 3;
        System.out.println(solution(votes, remainder));
    }
    ...
    static int solution(int[] votes, int k) {
        ...
        int result = 0;
        if (k == 0 && votes[votes.length - 2] != votes[votes.length - 1]) result = 1;
        for (int w : votes) {
