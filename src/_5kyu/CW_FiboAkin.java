...
We have u[7] = 5 and u[6] = 4. These numbers tell us that we have to go backwards from index 8
to index 8 - 5 = 3 and to index 8 - 4 = 4 so to index 3 and 4.
...
Given two numbers n, k (integers > 2) write the function length_sup_u_k(n, k) or lengthSupUK or length-sup-u-k
returning the number of terms u[i] >= k with 1 <= i <= n. If we look above we can see that between u[1]
and u[23] we have four u[i] greater or equal to 12: length_sup_u_k(23, 12) => 4
Given n (integer > 2) write the function comp(n) (cmp in COBOL) returning the number of times where a term
...
public class CW_FiboAkin {
    public static void main(String[] args) {
        ...
        System.out.println(lengthSupUK(500, 100)); // 304

        System.out.println(comp(23)); // 1
        System.out.println(comp(100)); // 22
        System.out.println(comp(200)); // 63
    }
