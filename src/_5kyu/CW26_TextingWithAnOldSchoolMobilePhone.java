...
and sending your first text message with excitement in your eyes. Maybe you still have one lying in a drawer somewhere.

Let's try to remember the good old days and what it was like to send text messages with such devices.
A lot of them had different layouts, most notably for special characters and spaces, so for simplicity we'll
be using a fictional model with 3x4 key layout shown below:
...
The task
You got your thumb ready to go, so you'll receive a message and your job is to figure out which keys you need
to press to output the given message with the lowest number of clicks possible. Return the result as a string
of key inputs from top row (refer to diagram above).
...
Typing letters
To type letters, press a button repeatedly to cycle through the possible characters (bottom row
of a key's layout). Pressing is represented by key's top row element repeated n times, where n is
the position of character on that particular key. Examples:
2 => 'a', 9999 => 'z', 111 => '?', *** => '+'
...
If you have 2 or more characters in a sequence that reside on the same button (refer to layout,
bottom row), you have to wait before pressing the same button again. Waiting is represented by adding
a space between 2 (or more) such characters. Example:
44 444 44 444 => 'hihi'
 ...
 */
public class CW26_TextingWithAnOldSchoolMobilePhone {
    public static void main(String[] args) {
        System.out.println(sendMessage("Def Con 1!")); // "#3#33 3330#222#666 6601-1111"
        System.out.println(sendMessage("D=b*b-4*a*c")); // "#3****#22*-22**4-*-2*-222"
    }

    public static String sendMessage(String message) {
        String buttons = "111122233344455566677778889999****0";
        ...
        for (int i = 0; i < message.length(); i++) {
            ...
            int index = chars.indexOf(String.valueOf(c).toLowerCase());
            char button = index == -1 ? c : buttons.charAt(index);
            if (prevButton == button && !Character.isDigit(message.charAt(i - 1))) {
                result.append(" ");
            }
            if (Character.isAlphabetic(c) || ".,?!'-+= ".contains(c + "")) {
                if ((Character.isUpperCase(c) && !isUpper) || (Character.isLowerCase(c) && isUpper)) {
                    isUpper = !isUpper;
                    if (i > 0 && result.charAt(result.length() - 1) == ' ') result.deleteCharAt(result.length() - 1);
                    result.append("#");
                }
                int count = index - buttons.indexOf(button) + 1;
                result.append(String.valueOf(button).repeat(count));
            } else if (Character.isDigit(c) || "*#".contains(c + "")) {
