...
"WEST" and "SOUTH", "SOUTH" and "EAST" are not directly opposite of each other and can't become such.
Hence the result path is itself : ["NORTH", "WEST", "SOUTH", "EAST"].
 */
public class CW55_DirectionsReduction {
    ...
    public static String[] dirReduc(String[] arr) {
        ...
        for (String s : arr) {
            if (!directionStack.isEmpty() && areOpposite(directionStack.peek(), s))
                directionStack.pop();
            else
                directionStack.add(s);
        }

        return directionStack.toArray(new String[0]);
    }

    static List<String> northSouth = new ArrayList<>(List.of("NORTH", "SOUTH"));
    static List<String> eastWest = new ArrayList<>(List.of("WEST", "EAST"));

    private static boolean areOpposite(String a, String b) {
        return !a.equals(b)
                &&
                ((northSouth.contains(a) && northSouth.contains(b)) || (eastWest.contains(a) && eastWest.contains(b)));
    }
}
