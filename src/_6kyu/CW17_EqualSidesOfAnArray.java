 ...
 */
public class CW17_EqualSidesOfAnArray {
    ...
    public static int findEvenIndex(int[] arr) {
        for (int i = 0; i < arr.length; i++) {
            if (helperMethod(arr, 0, i - 1) == helperMethod(arr, i + 1, arr.length - 1))
                return i;
        }
        return -1;
    }

    private static int helperMethod(int[] arr, int from, int to) {
        int sum = 0;
